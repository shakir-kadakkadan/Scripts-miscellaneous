#!/usr/bin/env python3
"""
NIFTY vs TATASTEEL - Instagram Reel Video Generator
...
def fetch_tatasteel_data():
    """Fetch TATASTEEL data from Yahoo Finance (adjusted for dividends, splits, bonuses)."""
    print("Fetching TATASTEEL data from Yahoo Finance...")
    ticker = yf.Ticker("TATASTEEL.NS")  # Tata Steel on NSE
    df = ticker.history(period="max")

    df = df.reset_index()
    df['date'] = pd.to_datetime(df['Date']).dt.tz_localize(None)
    df['tatasteel'] = df['Close']  # Yahoo Finance 'Close' is adjusted
    df['year_month'] = df['date'].dt.to_period('M')
...
def prepare_data():
    """Fetch and merge NIFTY and TATASTEEL data."""
    nifty_df = fetch_nifty_data()
    tatasteel_df = fetch_tatasteel_data()

    # Merge on year_month
    nifty_df['year_month'] = nifty_df['date'].dt.to_period('M')
    tatasteel_df['year_month'] = tatasteel_df['date'].dt.to_period('M')

    merged = pd.merge(
        nifty_df[['year_month', 'nifty']],
        tatasteel_df[['year_month', 'tatasteel']],
        on='year_month',
        how='inner'
    )
...
def create_reel_video(
    df: pd.DataFrame,
    output_path: str = "nifty_vs_tatasteel_reel.mp4",
    fps: int = 30,
    ...
    for i in range(len(df)):
        ...
        for j in range(i + 1):
            ...
            tatasteel_growth = df['tatasteel'].iloc[i] / df['tatasteel'].iloc[j]
            nifty_value += sip_amount * nifty_growth
            tatasteel_value += sip_amount * tatasteel_growth

        nifty_sip.append(nifty_value)
        tatasteel_sip.append(tatasteel_value)
        total_invested.append(invested)

    nifty_normalized = pd.Series(nifty_sip)
    tatasteel_normalized = pd.Series(tatasteel_sip)
    ...
    tatasteel_color = '#1e90ff'  # Dodger blue for TATASTEEL

    # Create line objects
    line_nifty, = ax.plot([], [], lw=3, color=nifty_color, label='NIFTY 50')
    line_tatasteel, = ax.plot([], [], lw=3, color=tatasteel_color, label='TATASTEEL')

    # Set axis limits
    x_data = np.arange(len(df))
    y_min = 0  # Start from 0 so graph is visible from beginning
    y_max = max(nifty_normalized.max(), tatasteel_normalized.max()) * 1.1
    ...
    tatasteel_box = ax.text(0.92, 0.93, '', transform=ax.transAxes,
                           fontsize=14 * text_scale, color=tatasteel_color, ha='center',
                           fontweight='bold',
                           bbox=dict(boxstyle='round,pad=0.4', facecolor='#1a1a1a',
                                    edgecolor=tatasteel_color, linewidth=2))
    ...
    nifty_pct_text = ax.text(0.08, 0.86, '', transform=ax.transAxes,
                            ...
                            fontweight='bold')
    tatasteel_pct_text = ax.text(0.92, 0.86, '', transform=ax.transAxes,
                                fontsize=13 * text_scale, color=tatasteel_color, ha='center',
                                fontweight='bold')
    ...
    tatasteel_label = ax.text(0, 0, '', fontsize=14 * text_scale, fontweight='bold',
                             color=tatasteel_color, va='center')
    ...
    def init():
        line_nifty.set_data([], [])
        line_tatasteel.set_data([], [])
        date_text.set_text('')
        invested_text.set_text('')
        sip_text.set_text('')
        nifty_label.set_text('')
        tatasteel_label.set_text('')
        nifty_box.set_text('NIFTY 50\n₹0')
        tatasteel_box.set_text('TATASTEEL\n₹0')
        nifty_pct_text.set_text('0%')
        tatasteel_pct_text.set_text('0%')
        return [line_nifty, line_tatasteel, date_text, invested_text, sip_text, nifty_label, tatasteel_label,
                nifty_box, tatasteel_box, nifty_pct_text, tatasteel_pct_text]
    ...
    def animate(frame):
        ...
        tatasteel_show = tatasteel_normalized.iloc[:n_show].values

        line_nifty.set_data(x_show, nifty_show)
        line_tatasteel.set_data(x_show, tatasteel_show)
        ...
        tatasteel_end = tatasteel_show[-1]
        ...
        tatasteel_label.set_position((x_end + offset, tatasteel_end))
        tatasteel_label.set_text(fmt_val(tatasteel_end))

        # Calculate total invested and % returns
        invested = invested_series.iloc[n_show - 1]
        nifty_pct = ((nifty_end - invested) / invested) * 100
        tatasteel_pct = ((tatasteel_end - invested) / invested) * 100
        ...
        tatasteel_box.set_text(f'TATASTEEL\n{fmt_val(tatasteel_end)}')

        # Update percentage labels (smaller text below boxes)
        nifty_pct_text.set_text(f'{nifty_pct:+.0f}%')
        tatasteel_pct_text.set_text(f'{tatasteel_pct:+.0f}%')
    ...
    writer = FFMpegWriter(fps=fps, metadata={'title': 'NIFTY vs TATASTEEL'}, bitrate=8000)
...
def main():
    ...
    df.to_csv('nifty_vs_tatasteel_data.csv', index=False)
    print(f"Data saved to: nifty_vs_tatasteel_data.csv")

    # Create video
    video_path = create_reel_video(
        df=df,
        output_path="nifty_vs_tatasteel_reel.mp4",
        fps=30,
        duration_seconds=16
    )
