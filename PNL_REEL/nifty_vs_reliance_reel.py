#!/usr/bin/env python3
"""
NIFTY vs RELIANCE - Instagram Reel Video Generator
...
def fetch_reliance_data():
    """Fetch RELIANCE data from Yahoo Finance (adjusted for dividends, splits, bonuses)."""
    print("Fetching RELIANCE data from Yahoo Finance...")
    ticker = yf.Ticker("RELIANCE.NS")  # Reliance Industries on NSE
    df = ticker.history(period="max")

    df = df.reset_index()
    df['date'] = pd.to_datetime(df['Date']).dt.tz_localize(None)
    df['reliance'] = df['Close']  # Yahoo Finance 'Close' is adjusted
    df['year_month'] = df['date'].dt.to_period('M')
...
def prepare_data():
    """Fetch and merge NIFTY and RELIANCE data."""
    nifty_df = fetch_nifty_data()
    reliance_df = fetch_reliance_data()

    # Merge on year_month
    nifty_df['year_month'] = nifty_df['date'].dt.to_period('M')
    reliance_df['year_month'] = reliance_df['date'].dt.to_period('M')

    merged = pd.merge(
        nifty_df[['year_month', 'nifty']],
        reliance_df[['year_month', 'reliance']],
        on='year_month',
        how='inner'
    )
...
def create_reel_video(
    df: pd.DataFrame,
    output_path: str = "nifty_vs_reliance_reel.mp4",
    fps: int = 30,
    ...
    for i in range(len(df)):
        ...
        for j in range(i + 1):
            ...
            reliance_growth = df['reliance'].iloc[i] / df['reliance'].iloc[j]
            nifty_value += sip_amount * nifty_growth
            reliance_value += sip_amount * reliance_growth

        nifty_sip.append(nifty_value)
        reliance_sip.append(reliance_value)
        total_invested.append(invested)

    nifty_normalized = pd.Series(nifty_sip)
    reliance_normalized = pd.Series(reliance_sip)
    ...
    reliance_color = '#0066b3'  # Reliance blue

    # Create line objects
    line_nifty, = ax.plot([], [], lw=3, color=nifty_color, label='NIFTY 50')
    line_reliance, = ax.plot([], [], lw=3, color=reliance_color, label='RELIANCE')

    # Set axis limits
    x_data = np.arange(len(df))
    y_min = 0  # Start from 0 so graph is visible from beginning
    y_max = max(nifty_normalized.max(), reliance_normalized.max()) * 1.1
    ...
    reliance_box = ax.text(0.92, 0.93, '', transform=ax.transAxes,
                          fontsize=14 * text_scale, color=reliance_color, ha='center',
                          fontweight='bold',
                          bbox=dict(boxstyle='round,pad=0.4', facecolor='#1a1a1a',
                                   edgecolor=reliance_color, linewidth=2))
    ...
    nifty_pct_text = ax.text(0.08, 0.86, '', transform=ax.transAxes,
                            ...
                            fontweight='bold')
    reliance_pct_text = ax.text(0.92, 0.86, '', transform=ax.transAxes,
                               fontsize=13 * text_scale, color=reliance_color, ha='center',
                               fontweight='bold')
    ...
    reliance_label = ax.text(0, 0, '', fontsize=14 * text_scale, fontweight='bold',
                            color=reliance_color, va='center')
    ...
    def init():
        line_nifty.set_data([], [])
        line_reliance.set_data([], [])
        date_text.set_text('')
        invested_text.set_text('')
        sip_text.set_text('')
        nifty_label.set_text('')
        reliance_label.set_text('')
        nifty_box.set_text('NIFTY 50\n₹0')
        reliance_box.set_text('RELIANCE\n₹0')
        nifty_pct_text.set_text('0%')
        reliance_pct_text.set_text('0%')
        return [line_nifty, line_reliance, date_text, invested_text, sip_text, nifty_label, reliance_label,
                nifty_box, reliance_box, nifty_pct_text, reliance_pct_text]
    ...
    def animate(frame):
        ...
        reliance_show = reliance_normalized.iloc[:n_show].values

        line_nifty.set_data(x_show, nifty_show)
        line_reliance.set_data(x_show, reliance_show)
        ...
        reliance_label.set_position((x_end + offset, reliance_end))
        reliance_label.set_text(fmt_val(reliance_end))

        # Calculate total invested and % returns
        invested = invested_series.iloc[n_show - 1]
        nifty_pct = ((nifty_end - invested) / invested) * 100
        reliance_pct = ((reliance_end - invested) / invested) * 100
        ...
        reliance_box.set_text(f'RELIANCE\n{fmt_val(reliance_end)}')

        # Update percentage labels (smaller text below boxes)
        nifty_pct_text.set_text(f'{nifty_pct:+.0f}%')
        reliance_pct_text.set_text(f'{reliance_pct:+.0f}%')
    ...
    writer = FFMpegWriter(fps=fps, metadata={'title': 'NIFTY vs RELIANCE'}, bitrate=8000)
...
def main():
    ...
    df.to_csv('nifty_vs_reliance_data.csv', index=False)
    print(f"Data saved to: nifty_vs_reliance_data.csv")

    # Create video
    video_path = create_reel_video(
        df=df,
        output_path="nifty_vs_reliance_reel.mp4",
        fps=30,
        duration_seconds=16
    )
