#!/usr/bin/env python3
"""
NIFTY vs INFY - Instagram Reel Video Generator
...
def fetch_infy_data():
    """Fetch INFY data from Yahoo Finance (adjusted for dividends, splits, bonuses)."""
    print("Fetching INFY data from Yahoo Finance...")
    ticker = yf.Ticker("INFY.NS")  # Infosys on NSE
    df = ticker.history(period="max")

    df = df.reset_index()
    df['date'] = pd.to_datetime(df['Date'])
    df['infy'] = df['Close']  # Yahoo Finance 'Close' is adjusted
    df['year_month'] = df['date'].dt.to_period('M')
...
def prepare_data():
    """Fetch and merge NIFTY and INFY data."""
    nifty_df = fetch_nifty_data()
    infy_df = fetch_infy_data()

    # Merge on year_month
    nifty_df['year_month'] = nifty_df['date'].dt.to_period('M')
    infy_df['year_month'] = infy_df['date'].dt.to_period('M')

    merged = pd.merge(
        nifty_df[['year_month', 'nifty']],
        infy_df[['year_month', 'infy']],
        on='year_month',
        how='inner'
    )
...
def create_reel_video(
    df: pd.DataFrame,
    output_path: str = "nifty_vs_infy_reel.mp4",
    fps: int = 30,
    ...
    for i in range(len(df)):
        ...
        for j in range(i + 1):
            ...
            infy_growth = df['infy'].iloc[i] / df['infy'].iloc[j]
            nifty_value += sip_amount * nifty_growth
            infy_value += sip_amount * infy_growth

        nifty_sip.append(nifty_value)
        infy_sip.append(infy_value)
        total_invested.append(invested)

    nifty_normalized = pd.Series(nifty_sip)
    infy_normalized = pd.Series(infy_sip)
    ...
    infy_color = '#4a90d9'   # Blue for INFY (Infosys brand color)

    # Create line objects
    line_nifty, = ax.plot([], [], lw=3, color=nifty_color, label='NIFTY 50')
    line_infy, = ax.plot([], [], lw=3, color=infy_color, label='INFY')

    # Set axis limits
    x_data = np.arange(len(df))
    y_min = 0  # Start from 0 so graph is visible from beginning
    y_max = max(nifty_normalized.max(), infy_normalized.max()) * 1.1
    ...
    start_year = df['date'].iloc[0].year
    subtitle = ax.text(0.5, 1.075, f'₹4K Monthly SIP +₹400/yr since {start_year}',
    ...
    infy_box = ax.text(0.92, 0.93, '', transform=ax.transAxes,
                      fontsize=17 * text_scale, color=infy_color, ha='center',
                      fontweight='bold',
                      bbox=dict(boxstyle='round,pad=0.4', facecolor='#1a1a1a',
                               edgecolor=infy_color, linewidth=2))
    ...
    nifty_pct_text = ax.text(0.08, 0.86, '', transform=ax.transAxes,
                            ...
                            fontweight='bold')
    infy_pct_text = ax.text(0.92, 0.86, '', transform=ax.transAxes,
                           fontsize=13 * text_scale, color=infy_color, ha='center',
                           fontweight='bold')
    ...
    infy_label = ax.text(0, 0, '', fontsize=14 * text_scale, fontweight='bold',
                        color=infy_color, va='center')
    ...
    def init():
        ...
        infy_box.set_text('INFY\n₹0')
        nifty_pct_text.set_text('0%')
        infy_pct_text.set_text('0%')
        return [line_nifty, line_infy, date_text, invested_text, sip_text, nifty_label, infy_label,
                nifty_box, infy_box, nifty_pct_text, infy_pct_text]
    ...
    def animate(frame):
        ...
        infy_show = infy_normalized.iloc[:n_show].values

        line_nifty.set_data(x_show, nifty_show)
        line_infy.set_data(x_show, infy_show)
        ...
        infy_label.set_position((x_end + offset, infy_end))
        infy_label.set_text(fmt_val(infy_end))

        # Calculate total invested and % returns
        invested = invested_series.iloc[n_show - 1]
        nifty_pct = ((nifty_end - invested) / invested) * 100
        infy_pct = ((infy_end - invested) / invested) * 100
    ...
    writer = FFMpegWriter(fps=fps, metadata={'title': 'NIFTY vs INFY'}, bitrate=8000)
...
def main():
    ...
    df.to_csv('nifty_vs_infy_data.csv', index=False)
    print(f"Data saved to: nifty_vs_infy_data.csv")

    # Create video
    video_path = create_reel_video(
        df=df,
        output_path="nifty_vs_infy_reel.mp4",
        fps=30,
        duration_seconds=16
    )
